...
/**
 * Optimized Gemini AI-Driven Style-Preserved Email Translation Engine
 * Bypasses Netlify 10-second timeouts by using full streaming responses.
 */
export async function processEmailTranslation(htmlSource, targetDict, sourceDict, brandCode, targetLang) {
  ...
  // 1. Pack translation entries into a clean reference array
  ...
  // 2. Build strict determinism system instructions
...
`;
  ...
  // 3. FIX: Changed to generateContentStream to keep data flowing and trick Netlify's timeout window
  ...
  // 4. Gather the chunks as they stream down from the AI engine
  let translatedHtmlOutput = '';
  for await (const chunk of responseStream) {
    translatedHtmlOutput += chunk.text;
  }

  // 5. Post-Sanitize markdown code fences wrapper formatting
  ...
  // 6. RUN CHEERIO FOR SFMC COMPATIBILITY INJECTION TRACKING ALIASES
