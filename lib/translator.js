...
`;
  ...
  $('a').each(function() {
    ...
    // Rule 2: App Store & Play Store Tracking Structure Route Capture
    if (alt.includes('app store') || alt.includes('appstore')) {
      anchor.attr('alias', 'App Store');
      anchor.attr('href', '%%=redirectto(@appstore)=%%');
      return;
    }
    if (alt.includes('play store') || alt.includes('playstore') || alt.includes('google play')) {
      anchor.attr('alias', 'Play Store');
      anchor.attr('href', '%%=redirectto(@playstore)=%%');
      return;
    }
    ...
    if (alt.includes('twitter') || alt.includes('x') || alt.includes('tw')) {
